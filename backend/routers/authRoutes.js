...
router.post('/sign-in', async (req, res) => {
    ...
    //check if email exists
    const user = await User.findOne({ email: email })
    //console.log(Object.keys(user._doc).has('adm'))
    console.log('teste: '+user._doc.hasOwnProperty("adm"))
    if(!user._doc.hasOwnProperty("adm")){
    ...
    if (user._doc.hasOwnProperty("adm") && Boolean(Object.entries(user._doc.adm)) === true) {
        console.log('admin conectado: ' + Boolean(Object.entries(user._doc.adm)))
        ...
        const token2 = jwt.sign(
            ...
            "VamosTodosMorrerDuasVezes"
        )
        return res.json({ error: null, msg: "voce está autenticado!", token: token, userId: user._id, token2: token2 })
